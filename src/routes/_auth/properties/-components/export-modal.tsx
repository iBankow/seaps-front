...
const AVAILABLE_COLUMNS = [
  { id: "id", label: "ID", essential: true },
  { id: "organization", label: "Orgão", essential: true },
  { id: "name", label: "Nome", essential: true },
  { id: "city", label: "Cidade", essential: false },
  { id: "address", label: "Endereço", essential: false },
  { id: "person", label: "Responsável", essential: false },
  { id: "type", label: "Tipo", essential: false },
  { id: "created_at", label: "Criado em", essential: false },
  { id: "updated_at", label: "Atualizado em", essential: false },
];
  ...
  const getPreviewValue = (item: any, columnId: string) => {
    switch (columnId) {
      case "id":
        return item.id;
      case "organization":
        return item.organization?.acronym || item.organization?.name || "--";
      case "name":
        return item.name || "--";
      case "city":
        return item.city || "--";
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const timestamp = new Date()
        ...
        .slice(0, 19)
        .replace(/[:.]/g, "-");
      link.download = `imoveis-export-${timestamp}.xlsx`;
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                {/* <Badge variant="secondary">
                  Mostrando {Math.min(5, previewData.length)} de{" "}
                  {data?.length || 0}
                </Badge> */}
              </div>
