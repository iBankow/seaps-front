export const BadNotification = ({ ref }: any) => {
  return (
    <div
      className="relative bg-white text-black font-serif p-[20mm]"
      style={{ width: "210mm" }}
      ref={ref}
    >
      <div>
        <section className="max-w-none text-justify mb-6 space-y-4">
          <p>
            Em atenção ao Decreto Estadual nº. 1.387, de 25 de março de 2025,
            que “Estabelece diretrizes para a manutenção e conservação dos
            imóveis ocupados pelos órgãos e entidades da Administração Direta e
            Indireta do Poder Executivo do Estado de Mato Grosso, e dá outras
            providências”, que definiu que a SEPLAG, enquanto órgão central de
            patrimônio, é responsável pela orientação e fiscalização da execução
            de ações preventivas, de manutenção e conservação de bens imóveis
            públicos do Estado de Mato Grosso, sirvo-me do presente para{" "}
            <strong>NOTIFICAR</strong> o responsável pela guarda, manutenção e
            conservação do bem imóvel objeto do{" "}
            <strong>Relatório de Vistoria</strong> anexado ao final deste
            ofício.
          </p>

          <p>
            A ação foi conduzida nos termos do art. 5º, caput, do Decreto
            Estadual nº 1.387, de 2025, que atribuiu à SEPLAG a incumbência de
            orientar e fiscalizar a execução das ações pertinentes:
          </p>

          <blockquote className="border-l-2 pl-4 italic">
            “Art. 5º Compete à Secretaria de Estado de Planejamento e Gestão -
            SEPLAG, órgão central de patrimônio estadual, orientar e fiscalizar
            a execução das ações de prevenção, de manutenção e conservação dos
            imóveis dos órgãos e entidades do Poder Executivo do Estado de Mato
            Grosso.
          </blockquote>
          <blockquote className="border-l-2 pl-4 italic">[...] </blockquote>
          <blockquote className="border-l-2 pl-4 italic">
            § 3º A autoridade máxima e o gestor da unidade administrativa do
            órgão ou entidade do Poder Executivo Estadual que, na fiscalização
            prevista no § 1º deste artigo, tiver o imóvel classificado como em
            situação "ruim" em qualquer dos itens do anexo único deste Decreto,
            além da determinação do § 2º deste artigo, será formalmente
            advertido pela não adoção das medidas corretivas.”
          </blockquote>

          <p>
            O relatório classificou o imóvel localizado na Rua Comandante Costa,
            nº 190, centro, Cuiabá -MT, onde funciona a Fundação de Amparo à
            Pesquisa do Estado de Mato Grosso em situação{" "}
            <strong>"RUIM"</strong>, nos termos do § 3º do art. 5º do Decreto nº
            1.387/2025.
          </p>

          <p>
            Diante do exposto, concede-se o prazo improrrogável até 03 de
            outubro de 2025 para que sejam adotadas as medidas corretivas
            cabíveis em relação às irregularidades apontadas, com a devida
            apresentação de evidências comprobatórias
          </p>

          <p>
            Esta notificação tem caráter meramente informativo, para ciência e
            controle interno da unidade responsável, não implicando neste
            momento a fixação de prazos ou penalidades. Entretanto, recomendamos
            que as medidas necessárias para o aperfeiçoamento das condições do
            imóvel sejam avaliadas e programadas, especialmente em observância
            ao art. 4º do Decreto nº 1.387/2025:
          </p>

          <p>
            O não atendimento da presente notificação implicará nova notificação
            formal e poderá ensejar a adoção imediata das medidas sancionatórias
            previstas no artigo 8º, do Decreto nº 1.387/2025, sem prejuízo de
            responsabilizações cíveis, administrativas e, se for o caso, penais
            cabíveis, vejamos:
          </p>

          <blockquote className="border-l-2 pl-4 italic">
            “Art. 8º No âmbito do Poder Executivo Estadual, o descumprimento das
            obrigações previstas neste Decreto poderá resultar em: <br />I -
            aplicação do regime orçamentário e financeiro cautelar ao órgão ou
            entidade que deixar de realizar as ações de prevenção ou de
            manutenção de imóveis sob sua responsabilidade;
            <br /> II - abertura de processo administrativo para apuração de
            responsabilidade da autoridade máxima e do gestor da unidade
            administrativa que, por ação ou omissão, tenha dado causa ou
            contribuído para a deterioração ou dano ao patrimônio público, sem
            prejuízo de outras penalidades cabíveis;
            <br /> III - redução do Indicador de Governança e Eficiência de Mato
            Grosso IGEF/MT, de que trata a Lei nº 12.771, de 20 de dezembro de
            2024.”
          </blockquote>

          <p className="mt-6">Sendo o que tinha a informar.</p>
        </section>
      </div>
    </div>
  );
};
